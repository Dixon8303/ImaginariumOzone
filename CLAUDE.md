...
Defined in `backend/pipeline/orchestrator.py` — the `stages` list is execution order,
`ROUTING` is the tier map. Both must stay in sync with `STAGE_ORDER` in
`backend/services/status_sync.py` and the `STAGES` array in `docs/bgf/index.html`.

| # | Stage key | Spec | Tier | Pre-work | Autonomy |
|---|-----------|------|------|----------|----------|
| 1 | `topic_scoring` | P1 Ideation/Score | Opus 4.8 | Ollama titles | CHECKPOINT |
| 2 | `research` | P2 Research | **Fable 5** | — | AUTO |
| 3 | `outline` | P3 Outline | Opus 4.8 | — | AUTO |
| 4 | `script` | P4 Script | Opus 4.8 | — | AUTO_REVIEW |
| 5 | `seo` | P8 SEO | Haiku 4.5 | Ollama tags | AUTO |
| 6 | `voice_ssml` | P5 Voiceover | Sonnet 4.6 | — | AUTO |
| 7 | `asset_planning` | P6 Storyboard | Sonnet 4.6 | Ollama shots | AUTO |
| 8 | `generation` | Asset generation | ComfyUI (Wan2.2) | — | AUTO |
| 9 | `ken_burns` | Motion pass | ffmpeg | — | AUTO |
| 10 | `assembly` | Episode assembly | ffmpeg | — | AUTO |
| 11 | `shorts` | P9 Shorts | Sonnet 4.6 | Ollama captions | AUTO |
| 12 | `qa_gates` | P10 QA + G3–G5 | Opus 4.8 | code checks | CHECKPOINT |

Model IDs are centralized in `backend/config.py` (`MODEL_FABLE`, `MODEL_OPUS`,
`MODEL_SONNET`, `MODEL_HAIKU`). Never hard-code a model string in a service or router.

**Tier discipline:** the tier assignment is the product, not an implementation detail.
Research goes to Fable because claim verification is the highest-stakes step; SEO goes
to Haiku because it is mechanical. Do not "upgrade" a stage to a larger model to fix a
bad output — fix the prompt in `backend/prompts/` first.

### Gates G1–G5

Evaluated by Opus in `backend/services/claude_client.py`, persisted to
`gate_decisions`. A FAIL raises `HardHalt`, writes a remediation row, and stops the
...
| **G4** Predictive | P10 | CTR + retention likelihood vs. benchmarks (CTR ≥6%, retention ≥30% first-mark) | Opus verdict |
...
Every mode pauses before upload. There is no unattended-publish path.
...
- **Keep the four stage lists in sync** (orchestrator `stages`, orchestrator `ROUTING`,
  ...
  all four plus `BGF_USER_GUIDE.md`.
