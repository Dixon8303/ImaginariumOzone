...
then `heat = max(fx_mul(cost, FX_ENTROPY_TAX), 1)` is booked to the Chunk ledger `dissipated` (DEOS-CORE REQ-LAW-005) and `SPAWN` is issued with `payload[0] = cost − heat`, `payload[1..2]` = parent position, `payload[3]` = archetype in bits 0–15 and `min(parent.generation + 1, 65535)` in bits 16–31, `aux` = the SpawnPayload reference. The parent appends Memory kind `11 BORN` (Agents; REQ-COG-009). A Genome whose mutated `gene[15]` (`mutation_rate_base`) or any parameter row moved by more than `MUT_STEP × 2` from the parent's emits `GENOME_DRIFT` (REQ-EVT-002).
...
Entries `4 ATTACK`, `12 EXPLORED`, `13 FLED` are appended by the rules that perform them in the same pass (REQ-COG-005, 007); `11 BORN` is appended when `A.Decision.action == REPRODUCE` and `B.current_energy` fell by the reproduction cost. Because `Memory` is owned by Stage 4 (DEOS-ECS REQ-CMP-013), Stage 5 outcomes are remembered by Stage 4 of the next tick from `A.Decision` and the observable result: a `TRADE` after which energy rose → `5 GIFT_RECEIVED`, fell → `6 GIFT_GIVEN`; `SHARE` → `7 SHARED`; `JOIN` with `A.Affiliation` now valid → `8 JOINED`; `LEAVE` → `9 LEFT`; `FOUND` with `A.Affiliation.role == FOUNDER` → `10 FOUNDED`. Memory feeds utility through `gain_k` (section 7.2) with salience `gene_param_trait(28)`.
