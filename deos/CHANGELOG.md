...
- **Hierarchy.** Seven modules in dependency order: DEOS-Foundation → DEOS-Core → DEOS-ECS → DEOS-Runtime → DEOS-Protocol → DEOS-Play → DEOS-MVS (`DEOS.md` section 2). The eight EESS specification directories collapse into `docs/01_Core/`, `docs/02_ECS/`, `docs/03_Runtime/`, and `docs/06_Prototype/`; the absorbed EESS documents were carried as `LEGACY_*.md` lineage files during drafting and removed once every legacy requirement was defined in its absorbing module (they remain in git history).
...
- **Test plan**: TS-SPEC-004 (identifier lint), TS-BENCH-003 (Epoch catch-up), TS-PLAY-001 (legibility), TS-PLAY-002 (event cadence), TS-PLAY-003 (seed code and fold vectors), per-requirement `TEST_<PREFIX>_<nnn>` naming.
- **Module drafts**: DEOS-CORE (47 requirements), DEOS-ECS (45), DEOS-RT (29), DEOS-PROTO (28), DEOS-PLAY (23), DEOS-MVS (9), each following the 13-section template with a player-facing consequence per mechanism.
- **ADRs**: ADR-0001 two-tier state hashing (per-tick Tick Hash over write folds; Checkpoint Hash over full Chunk Hashes at Checkpoint ticks); ADR-0002 substrate insolation (REQ-LAW-013, a renewable input so worlds persist through offline Epochs; `Ledger` gains `e_sun` and `heat_sink`).
- **`tools/gen_rtm.py`**: generates `docs/architecture/REQUIREMENT_TRACEABILITY_MATRIX.md` from the requirement headings.
- **Integration reconciliations**: the entropy tax is applied at Command barriers and every Command-side or own-write loss is credited to `Ledger.heat_sink` so the Stage 6 conservation check closes exactly; `ChunkLedger` gains birth and death counters for `WorldMood`; `Decision.flags` bit 2 marks exploratory choices; `InstitutionState.discovered` records technologies.
